...
To facilitate the effective development and communication of our team we have to have a certain amount of central infrastructure around.
We have opted to host our own gitlab instance and the server will have to be updated to keep up with the demands of the user.
With the use of gitlab we gain version control, effective issue tracking and basic tools for tracking the invested work.
To keep the quality of our software at a high level and to keep the turnaround time until a bug is diagnosed and fixed low we employ 
automated tests and profiling. These tests run on additional, separate machines that will have to be maintained. In addition we foresee the need
for virtualized environments so that we the availability to quickly test our software in different environments.
Use cases include the software environments of our users as well as those available at the supercomputer centers.
To be able to continually provide those services to our users(this includes the developers of the package as well as the users) 
at a high level we foresee hardware costs of 25 K\euro. \\
