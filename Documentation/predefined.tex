...
The $SU(N)$ Hubbard interaction on a given site $x $ is given by 
\begin{align}
...
To relate to  Eq.~\eqref{eqn:general_ham_v} we have,   $V_{x y}^{(ks)} =  \delta_{x,y} \delta_{x,k}$, $\alpha_{ks} = -\frac{1}{2}$ and $U_{k} =  \frac{U}{N_{\mathrm{col}}}$. 
