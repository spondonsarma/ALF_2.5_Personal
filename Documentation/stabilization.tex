...
\begin{itemize}
	...
	\item the propagation $e^{-\hat{H}}$ for one step $\Delta \tau$ for operators $\hat{T}= \sum_{n=1}^{N_T} \hat{T}_n$ and $\hat{V}= \sum_{n=1}^{N_V} \hat{V}_n$ reads:
	\begin{align}
	...
	\end{align}
	%\item the propagation for one step $\Delta \tau$ of a kinetic operator $\hat{T}= \sum_{n=1}^{N_T} \hat{T}_n$ reads:
	%\begin{align}
	%\prod_{n=N_T}^{1}e^{-\frac{\Delta \tau}{2} \hat{T}_n}  \prod_{n=1}^{N_T}e^{-\frac{\Delta \tau}{2} \hat{T}_n},
	%\end{align}
	%and similarly for an interaction term $\hat{V}= \sum_{n=1}^{N_V} \hat{V}_n$.
\end{itemize}
