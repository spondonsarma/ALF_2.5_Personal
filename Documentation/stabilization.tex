...
In order to circumvent this, more sophisticated methods have to be employed. As a first step, assuming that the multiplication of \texttt{NWrap} $\bm{B}$ matrices has an acceptable condition number and, for simplicity, that \texttt{NWrap} is a divisor of $L_{\text{Trotter}}$, we can write: \mycomment{(JP: I made $i \in (1, L_\text{Trotter}/\texttt{NWrap})$, instead of $i \in (0, L_\text{Trotter}/(\texttt{NWrap}-1))$ {\small and put $i-1$ instead of $i$ in $\bm{B}$})}
%\begin{equation}
