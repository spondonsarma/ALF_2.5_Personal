...
\end{equation}
Here the decomposition follows the rule  that if $k$ and $k'$  belong to the same set $\mathcal{S}^{T}_i $ then   $ \left[ \hat{T}^{(k)} , \hat{T}^{(k')} \right] = 0 $.  An important example is the checkerboard decomposition.
...
To address the issue described above, the ALF library provides the possibility to use a symmetric Trotter decomposition, 
\begin{equation}
...
In order to circumvent this, more sophisticated methods have to be employed. As a first step, assuming that the multiplication of \texttt{NWrap} $\bm{B}$ matrices has an acceptable condition number and, for simplicity,
that $L_{\text{Trotter}}$ is divisible by \texttt{NWrap}, we can write:
%\begin{equation}
