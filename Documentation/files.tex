...
\begin{table}[h]
   ...
   \path{data.h5}     & Contains the same information as the scalar, equal-time correlation and time-displaced correlation operators as in Tab.~\ref{table:output}, but in one single HDF5 file. Additionally saved are all Hamiltonian parameters that are defined as specified in Seq. \label{sec:hamiltonian} (see also Fig.~\ref{fig_data_h5}). Note: The parameter names in the HDF5 file are all lower case.\\
...
.3 sign\DTcomment{Dataset of shape (NBins)}.
.2 parameters.
.3 namelist\_x\DTcomment{Attached attributes are the parameters in namelist\_x}.
.3 namelist\_y\DTcomment{Attached attributes are the parameters in namelist\_x}.
}
\caption{Structure of HDF5 output file \texttt{data.h5}.}\label{fig_data_h5}
\end{figure}
