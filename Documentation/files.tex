...
It is important to be aware of the fragility of the numerical stabilization and that there is no guarantee that it will work for a given model. It is therefore crucial to always check the file \texttt{info}, which apart from runtime information, lists important information concerning the stability of the code, such as \texttt{Precision Green}.

If the numerical stabilization fails, [IDEA: INCLUDE 'rules of thumb' from notes-ALF.md: ``possible measures include: <table>"] one possible measure is to reduce the value of the parameter \texttt{Nwrap} in the parameter file. 
For performing the stabilization of the involved matrix multiplications we rely on routines from LAPACK, and results are very likely to change significantly if a different LAPACK implementation is used.
In order to offer a simple baseline to which people can quickly switch if they want to see whether their results depend on the library used for linear algebra routines, we have included parts of the LAPACK-3.6.1 reference implementation from \url{http://www.netlib.org/lapack/}. You can switch to the QR decomposition related routines from the LAPACK reference implementation by including the switch \texttt{-DQRREF} into the PROGRAMCONFIGURATION string in the script \path{configureHPC.sh}, described in Sec.~\ref{sec:running}.
...
\texttt{-DSTAB1} (stabilization scheme based on singular value decomposition) in the header of the file \texttt{Makefile} [CORRECT THAT, NOW IT'S PROBABLY IN configureHPC] and recompile the code.  
