...
\end{equation}
In the above,  $i = (\ve{i}, \delta_i) $  and $j = (\ve{j}, \delta_j) $  are super-indices encoding the unit-cell and orbital.   For simplicity, the interaction is specified with two  parameters, $U$ and $\alpha$ that monitor the  strength of the onsite interaction and the magnitude of the Coulomb tail  respectively.  
\begin{equation}
	V_{i, j}     \equiv = V(\vec{i}  + \vec{\delta}_i ,  \vec{j}  + \vec{\delta}_j  )   U \left\{
	\begin{array}{ll}  
	1          &   \text{ if }  i = j    \\
	\frac{\alpha   \;   d_\mathrm{min}}{  {  |  \overline{\vec{i} - \vec{j}} + \vec{\delta}_i - \vec{\delta}_j  |}   } &     \text{ otherwise }
	\end{array}
\right.
\end{equation}
Here $d_\mathrm{min}$ is the minimal distance between two orbitals.  Since  we use periodic boundary conditions   $\overline{\vec{i} - \vec{j}}$  is an element of the real space lattice. Note that this is encoded in the array \texttt{Latt\%imj(I,J)}.    The code uses the following  HS decomposition:
\begin{equation}
