...
\begin{table}[!ht]
	\begin{center}
		\begin{tabular}{@{} p{0.16\columnwidth} p{0.81\columnwidth} @{}}\toprule
...
\end{lstlisting}
The environment variable \texttt{ALF\_SHM\_CHUNK\_SIZE\_GB} can be used to reduce the memory footprint by sharing memory between MPI processes on the same node.
The environment variable defines the chunk size of the shared memory objects in units of GB, only positive integer values are supported.
Typical values are 1 or 2 GB, but they can be increased if the total number of MPI communicators is too large such that they trigger MPI error messages.
If \texttt{ALF\_SHM\_CHUNK\_SIZE\_GB} is not defined or set to values smaller that one, then the memory is not shared between MPI processes.
