...
In case the parameter \path{N_auto} is set to a finite value the program will also trigger the computation of autocorrelation functions (Sec.~\ref{sec:autocorr}).
...
\begin{table}[h]
	...
	\begin{tabular}{@{} p{0.15\linewidth} p{0.8\linewidth} @{}}\toprule
		...
		& Here \texttt{X}  is a place holder from \texttt{Green}, \texttt{SpinXY}, etc., as specified in \texttt{ Alloc\_obs(Ltau)} (See section \ref{Alloc_obs_sec}). Each directory contains  a  file    \texttt{g\_dat}  containing the  time-displaced correlation function traced over the  orbitals.  It also contains the covariance matrix if \texttt{N\_cov} is set to unity in the parameter file (see Sec.~\ref{sec:input}). Also, a directory \texttt{X\_R0} for the local time displaced correlation function is generated.  \\                         
