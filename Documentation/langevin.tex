...
\begin{equation}\label{eqn:langevin}
   \ve{s}(\tl +  \delta \tl)    =    \ve{s}(\tl)    - \frac{\partial S( \ve{s}(\tl)) }{\partial    \ve{s}(\tl) }    \delta \tl     +\sqrt{2 \delta \tl } \ve{\eta}(\tl).
\end{equation}
...
\end{equation}
and we consider an arbitrary real action  $S( \ve{s}(\tl))$. 
...
\end{equation}
where $\delta$ corresponds to the $L_\mathrm{trot} M_I $  dimensional Dirac $\delta$-function.   Taylor expanding  up to order $\delta \tl$  and averaging over the stochastic variable yields:
\begin{align}
...
Hence, to  formulate  the Langevin dynamics  we need  to estimate the forces:
\begin{equation}
...
\end{equation}
where  $\text{ sign }(C_n)$   is defined in Eq.~\ref{Sign.eq}
...
We have tested the code for a 6-site Hubbard chain at half-filling  at $U/t = 4$,  $\beta t = 4$.
