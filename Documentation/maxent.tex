...
\end{equation} 
The  ALF package includes a standard implementation of the stochastic MaxEnt as formulated in the article of K. Beach~\cite{Beach04a}. Here we will comment on its workflow.

\subsection{General setup}
The module  \texttt{Libraries/Modules/\allowbreak{}maxent\_stoch\_mod.F90} contains a general implementation and its wrapper is in \texttt{Analysis/Max\_SAC.F90}. 
...
\begin{equation}
 ...
 \end{equation}
where  $ \delta =  \Delta \omega$ with $ \Delta \omega = (\omega_\mathrm{end} -  \omega_\mathrm{start})/$\texttt{Ndis} and the image corresponds to that of the file \texttt{Aom\_ps\_m} with $ m = N_{\alpha} -10 $. 
...
\begin{equation}
	G^{\mathrm{ret}} (k, \omega)  = -i \int d t \Theta(t)  e^{i \omega t} \langle \left\{ \hat{c}^{\phantom\dagger}_{k} (t), \hat{c}^{\dagger}_{k} (0) \right\} \rangle
\end{equation}
...
\begin{equation}
	\int {\text d} \omega 	\coth(\beta \omega/2) \chi''(q,\omega) = 
...
\end{equation}
In the code, $\omega_\mathrm{start}$ is set to zero by default and the Kernel $K_{ph}$ is used in the code and is defined in the  routine \texttt{XKER\_ph}. 
