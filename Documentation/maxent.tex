...
To conclude this section, we   show the example of the one dimensional Hubbard model that is know to  show spin-charge separation (see Ref.~\cite{Abendschein06}  and references therein).    The data of Fig.~\ref{Fig:Spectral1D}    was produced with the pyALF python script  \href{}{\texttt{Spectral\_1D.pdf}}, and   the  spectral functions plots with the bash \href{}{\texttt{Spectral.sh}}. 

\begin{figure}
\center
\includegraphics[width=\textwidth]{Figures/MaxEnt/Spectral_1D.pdf}

\caption{ Dynamics of the one-dimensional half-filled Hubbard model  on a 46-site chain   and at U/t=4 and at $\beta t = 10$.  (a) Dynamical charge structure factor, (b) single particle spectral function and (c) dynamical spin structure factor.   As apparent in the pyALF python script \href{}{\texttt{Spectral\_1D.pdf}} we consider 400  bins each of 200 sweeps and  take into account the covariance matrix for the MaxEnt.  The parameters for the MaxEnt  that differ for the default values are also listed in the  python script. }
