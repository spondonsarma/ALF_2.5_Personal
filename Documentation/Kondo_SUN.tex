...
with
\begin{equation*}
	   \hat{D}^{\dagger}_{\ve{i}}   =  \sum_{\sigma=1}^{N} \hat{c}^{\dagger}_{\ve{i},\sigma}  \hat{f}^{\phantom\dagger}_{\ve{i},\sigma}   \text{  and  }   \hat{n}^{f}_i =   \sum_{\sigma=1}^{N}    \hat{f}^{\dagger}_{i,\sigma}   \hat{f}^{\phantom\dagger}_{i,\sigma}.
\end{equation*}
Constraining the  equation to the physical space, $\hat{n}^{f}_i = N/2 $.
\begin{align}
	 \frac{2 J}{N} \sum_{i, a=1  }^{N^2 -1}  \hat{T}^{a,c}_{i}  \hat{T}^{a,f}_{i}   =     -  \frac{J}{4N}    \left[ \left(   \hat{D}^{\dagger}_{i}  + \hat{D}^{\phantom\dagger}_{i}    \right)^{2}  + 
