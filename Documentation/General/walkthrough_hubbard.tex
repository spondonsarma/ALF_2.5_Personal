...
\begin{equation}
	\texttt{GRC(x,y,s)}   =  \delta_{x,y}  - \texttt{GRC(y,x,s)  }
\end{equation}
such that \texttt{GRC(x,y,s)}    corresponds to  $ \langle \langle  \hat{c}_{\vec{x},\sigma}^{\dagger}\hat{c}_{\vec{y},\sigma}^{\phantom\dagger} \rangle \rangle $.  
\begin{lstlisting} 
...
\FAcomment{FA.  I  plan to write the following. SU(2) Hubbard on Honeycomb lattice.  MZ Hubbard. Both will be rather short.  I will also extend the Hubbard model module to include the Honeycomb lattice. }
