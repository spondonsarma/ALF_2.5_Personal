   ...
   \frac{
     \left\{
      \sum_{C}  \left| \Re \left[e^{-S(C)} \right]  \right|   \text{ sign }(C)   \frac{e^{-S(C)}} {\Re \left[e^{-S(C)} \right]}  \langle \langle \hat{O}  \rangle \rangle_{(C)}  \right\}/
          ...
          &=&
  	 \frac{\sum_{C}   \langle  \text{ sign }(C)   \frac{e^{-S(C)}} {\Re \left[e^{-S(C)} \right]}  \langle \langle \hat{O}  \rangle \rangle_{(C)}  \rangle_{\overline{P}} } { \langle \text{sign} (C)   \rangle_{\overline{P}}}  \;.      
\end{eqnarray} 
Here,
\begin{equation}
\label{Sign.eq}
...
\end{equation}
which is real per definition.     In the subroutines  \texttt{Obser}  and \texttt{ObserT}  the main program will provide  the Green functions as well as the 
\begin{equation}
