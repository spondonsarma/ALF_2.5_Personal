...
${\hat{\mathcal{H}}=\hat{\mathcal{H}}_{T}+\hat{\mathcal{H}}_{V}} +  \hat{\mathcal{H}}_{I} +   \hat{\mathcal{H}}_{0,I}  $, where
\begin{eqnarray}
...
\begin{itemize}
\item The number of fermion \textit{flavors} is set by $N_{\mathrm{fl}}$.  After the Hubbard Stratonovitch transformation, the action will be block diagonal in the flavor index. 
...
Z &=& \Tr{e^{-\beta \hat{\mathcal{H}} }}\nonumber\\
  &=&   \Tr{  \left[ e^{-\Delta \tau \hat{\mathcal{H}}_{0,I}}   \prod_{k=1}^{M_T}   e^{-\Delta \tau \hat{T}^{(k)}}  
   ...
   &\quad&
    \Trf{   \prod_{\tau=1}^{L_{\mathrm{Trotter}}} \left[   \prod_{k=1}^{M_T}   e^{-\Delta \tau \hat{T}^{(k)}}  
...
\begin{eqnarray}
 &\quad&\Trf{   \prod_{\tau=1}^{L_{\mathrm{Trotter}}} \left[   \prod_{k=1}^{M_T}   e^{-\Delta \tau \hat{T}^{(k)}}  
    \prod_{k=1}^{M_V}   e^{  \sqrt{ \Delta \tau  U_k} \eta_{k,\tau} \hat{V}^{(k)} }   \prod_{k=1}^{M_I}   e^{  -\Delta \tau s_{k,\tau}  \hat{I}^{(k)}}  \right] } = \nonumber\\
...
The fundamental data structure in the code is the derived data type \texttt{Operator} which one uses to define the Hamiltonian.   
\begin{table}[h]
   \begin{tabular}{l l}
...
Consider the  matrix   ${\bm M}$ of dimension  $N_{\mathrm{dim}} \times N_{\mathrm{dim}} $.  Let us  denote  with  $ \left\{z_{1},\cdots,  z_{N}  \right\}$  a subset  of $N$ indices,  
for which
\begin{equation}
M_{x,y}  =
...
In general, we will not only have one structure variable \texttt{Operator}, but a whole  array of these structures, which defines the very Hamiltonian. 
...
\item In case of a full interaction (perfect-square term and Ising term), we  define  the array \texttt{Op\_V(M$_V$+M$_I$,N$_{fl}$) }) and set 
