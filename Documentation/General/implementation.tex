...
\begin{table}[h]
   ...
   The highlighted variables have to be specified by the user. \mycomment{Would be nice to reduce the complex type of  $g$ and $\alpha$ to real in the code.}
...
\end{equation}
\FAcomment{ $ \sum_{\vec{i},\vec{j}}$ is correct and is what is done in the code. This is actually why you then have to divide by  $N_{unit \,cell}$    }
...
\begin{table}[h]
   \begin{tabular}{l l l l }
    ...
    \texttt{Obs\%Phase}               &  Complex     &    Cumulated sign [Eq.~(\ref{Sign.eq})] & $\text{sign}(C)$  \\
...
\begin{table}[h]
   \begin{tabular}{l l l l }
    ...
    \texttt{Obs\%Phase}\footnote{Note that for historic reasons this variable is called \texttt{phase} although in the present implementation it does not refer to the (complex) Monte Carlo phase [Eq.~(\ref{eqn:phase})] but to the (integer) 
    sign [Eq.~(\ref{Sign.eq})] }
    &  Compl.  &    Cumulated sign [Eq.~(\ref{Sign.eq})] & $\text{sign}(C)$  \\
...
\FAcomment{FA.  The code runs in real space. Hence I think that it is much nicer to write things down  in real space. }
