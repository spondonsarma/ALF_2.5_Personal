...
We also obtain improved estimators by taking measurements in the imaginary-time intervall \texttt{[LOBS\_ST,LOBS\_EN]}  (see the parameter file in Sec.~\ref{??}) thereby exploiting the invariance under translation in imaginary time.
...
\begin{table}[h]
   \begin{tabular}{l l l l }
    ...
    \texttt{Obs\%Obs\_vec(:)}        & Compl.      &    Cumulated vector of observables [Eq.~(\ref{eqn:obs_rw})] &
     ...
     \texttt{Obs\%File\_Vec}           &  Char.    &    Name of output file  &
   \end{tabular}
...
\begin{eqnarray}
  ...
  &=&
   \frac{1}{N_{unit\, cell }} \sum_{\vec{i},\vec{j}}  e^{- i\vec{k} \cdot \left( \vec{i}-\vec{j}\right) }  \langle \hat{O}_{\vec{i},\alpha} (\tau) \hat{O}_{\vec{j},\beta} \rangle\label{eqn:s_corr}\;,\\
