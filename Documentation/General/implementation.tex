...
\end{table}
%
The \texttt{Lattice}  module equally handles  the Fourier transformation. 
\mycomment{
\begin{itemize}
\item  Mention Fourier transform and   other usefull subroutines in \texttt{Lattice\_v3}. 
\item  Give an example of  a  lattice or of a tilted lattice. 
\end{itemize}}
...
\begin{equation}
	S_{\alpha,\beta}(\vec{k}) = \frac{1}{N_{unit \;  cells}} \sum_{\vec{i},\vec{j}} e^{i \vec{k}\cdot (\vec{i} -  \vec{j} ) } \left( \langle \hat{O}_{\vec{i},\alpha}  \hat{O}_{\vec{j},\beta} \rangle  - 
