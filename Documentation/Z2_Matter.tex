...
Since flipping  $\hat{\tau}^{z}_{ \ve{i}} $  amount to changing the sign of the four  bond variables emanating from site $\ve{i}$,  the identitiy:
\begin{equation}
...
and 
 ...
 \end{align*}
In the above, $  | \underline{s}_{L_\tau+1}  \rangle  = | \underline{s}_{1}  \rangle   $ and a further  HS transformation of the Hubbard term (see Sec.~\ref{Hubbard_SUN_HS.eq})  the model is readily implemented in the ALF.    Including this HS field, $l$,  yields  the configuration space: 
\begin{equation}
...
\subsubsection{A test case: $Z_2$ slave spin formulation of the Hubbard model }
