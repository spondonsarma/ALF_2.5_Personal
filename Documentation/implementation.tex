...
\begin{table}[h]
    ...
    & \ref{sec:s0} \\
    \hl{\texttt{Global\_move}}        & Generates a global move and returns \texttt{T0\_Proposal\_ratio} (see Eq.~\eqref{T0_ratio}). 
    & \ref{sec:s0} \\
   ...
   \end{tabular}
   \caption{Overview of the subprograms of the  module \texttt{Hamiltonian}, contained in \texttt{Hamiltonian\_Examples\_mod.F90}, used to define the Hamiltonian, the lattice and the observables. The \hl{highlighted} subroutines may have to be modified by the user. \red{COMPLETE TABLE -- it should list all subroutines.}
...
\begin{table}[h]
	\begin{tabular}{@{} l l l @{}}\toprule
		...
		\texttt{Field\%i}         &  Integer & Returns \texttt{Field\%f} rounded to nearest integer (for \texttt{t=1} or \texttt{2}). \\
		...
		\texttt{Field\%amplitude} &  Real    & Width of a random flip for fields of type \texttt{t=3}, defaults to \texttt{1}. \\\bottomrule
	\end{tabular}
	\begin{tabular}{@{} l l @{}}\toprule	
		Function/Routine       & Description   \hspace{107.8mm}  \\\midrule
