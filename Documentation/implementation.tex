...
The generic term of the partition function (see Sec.~\ref{sec:method}) is written in terms of fields $\gamma$ and $\phi$ as
\begin{align}
\gamma e^{g \phi \bm{X} },
\end{align}
where $\bm{X}$ denotes an arbitrary operator and $g$ is a constant


For a Ising term, we store \path{type=1}, which sets $\phi_{k\tau}=s_{k\tau}$. 
In the case of a perfect-square term, the field results from the discrete HS transformation (\ref{HS_squares}) and we store \texttt{type=2}, which sets $\phi_{k\tau}=\eta_{k\tau}$. For continuous real fields $f$ we store \path{type=3}, which sets $\phi_{k\tau}=f$
