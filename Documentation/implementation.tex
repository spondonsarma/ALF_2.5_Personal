...
!#PARAMETERS END#
\end{lstlisting}
Each of those ``namelist specifications'' starts with a line containing \path{#PARAMETERS START#} and end with a line containing \path{#PARAMETERS END#}. The namelist name has to be written after \path{#PARAMETERS START#} on the same line. The variable type specification \path{<varX_type>} has to contain \path{real}, \path{complex}, \path{integer}, \path{character} or \path{logical}, which will be parsed as \path{real(Kind=Kind(0.d0))}, \path{integer}, \path{character(len=64)} or \path{logical} respectively. Each variable needs to have a default value, where the default vaule for complex numbers needs to be in the form \path{(real_part, imag_part)}. The description of the parameters is optional. A variable can be commented out, but will still be parsed to be read from parameters. This is to facilitate reading of variables that are already defined in \path{Hamiltonian_main}, e.g. \path{N_SUN}.
...
!#PARAMETERS END#
\end{lstlisting}
The parsing script \path{parse_ham.py} in \path{Prog} has the option \path{--test_file} for testing the namelist specifications, e.g. by calling:
