...
{\color{red} FFA.  Dear Jefferson, please do include the arguments in the  methods of the field  type.     Thanks, this is much better.  However, some arguments are optional, and some not.  For instance if  \texttt{In\_field}   is present,  then  the initial configuration will be set to the user defined configuration. If it is not present then a random one will be generated.   This is important information that should be part of the documentation. } 

%
\begin{table}[h]
	\begin{center}
	\begin{tabular}{@{} p{0.25\columnwidth} p{0.12\columnwidth} p{0.57\columnwidth} @{}}\toprule
		...
		\multicolumn{2}{@{}l@{}}{ \texttt{Fields\_init (del)}}               & Initializes internal variables such as $s_{k,\tau}$ and $\eta_{k,\tau}$, $\texttt{del}=\Delta x$.  \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%make (this,n\_op,n\_tau)}}  & Reserves memory for the field.    \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%clear (this)}}              & Clears field from memory.    \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%set (this)}}                & Sets a random configuration.    \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%flip (this,n\_op,n\_tau)}}  & Flips the field values randomly. \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%phi (this,n\_op,n\_tau)}}   & Returns $\phi_{k\tau}$ for the $k$-th operator at the time slice $\tau$. \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%gamma (this,n\_op,n\_tau)}} & Returns $\gamma_{k\tau}$ for the $k$-th operator at the time slice $\tau$.    \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%i (this,n\_op,n\_tau)}}     & Returns \texttt{Field\%f} rounded to nearest integer (for \texttt{t=1} or \texttt{2}). \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%in (this,Group\_Comm,In\_field)}} & Reads in the field configuration. \\
		\multicolumn{2}{@{}l@{}}{ \texttt{Field\%out (this,Group\_Comm)}}    & Writes out the field configuration.    \\\bottomrule
	\end{tabular}
	\caption{Components of a variable of type \texttt{Fields} named \texttt{Field}, where \texttt{this} denotes the field itself, \texttt{del} (real) denotes the \texttt{Field\%del}, \texttt{n\_op} and \texttt{n\_tau} (integers) are the number of operators and time slices, respectively, \texttt{Group\_Comm} (integer) defines an MPI communicator, and \texttt{In\_field} is the initial field configuration.}
    \end{center}
	\label{table:Fields}
\end{table}
%
