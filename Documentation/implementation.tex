...
!#PARAMETERS END#
\end{lstlisting}
Each of those ``namelist specifications'' starts with a line containing \path{#PARAMETERS START#} and end with a line containing \path{#PARAMETERS END#}. The namelist name has to be written after \path{#PARAMETERS START#} on the same line. The variable type specification \path{<varX_type>} has to contain \path{real}, \path{integer}, \path{character} or \path{logical}, which will be parsed as \path{real(Kind=Kind(0.d0))}, \path{integer}, \path{character(len=64)} or \path{logical} respectively. Each variable needs to have default value. The description of the parameters is optional. A variable can be commented out, but will still be parsed to be read from parameters. This is to facilitate reading of variables that are already defined in \path{Hamiltonian_main}, e.g. \path{N_SUN}.
...
The parsing script generates the file \texttt{Hamiltonian\_\emph{New\_model}\_read\_write\_parameters.F90} containing the subroutines \path{read_parameters} and \path{write_parameters_hdf5}. The former can be called in \path{ham_set}, while the latter has to be bound to \path{ham_New_model} through
\begin{lstlisting}[style=fortran]
#ifdef HDF5
...
For creating a new Hamiltonian, there is a template \path{Prog/Hamiltonians/Hamiltonian_##NAME##_smod.F90} to be copied to \texttt{Prog/Hamiltonians/Hamiltonian\_\emph{New\_model}\_smod.F90}.
