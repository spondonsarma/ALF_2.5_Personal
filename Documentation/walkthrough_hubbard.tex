...
\mycomment{Perhaps also mention \texttt{Call Op\_set(Op\_T(nc,n))} here}
...
fields corresponds to the Hubbard  interaction and the next \texttt{2*Ndim} ones to the Ising interaction.
\mycomment{Actually, in the above it's the other way around: The first \texttt{2*Ndim} fields correspond to the Ising interaction and the net \path{Ndim} ones to the Hubbard interaction.}
