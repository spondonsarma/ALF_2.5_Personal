...
\end{equation}
Here, the HS fields take the values $l_{j,\tau} = \pm 2, \pm 1 $ and, for $\hat{Z}_k$  corresponding to the Pauli matrix $\hat{\sigma}_{z}$, the Ising spins take the values $s_{i,\tau} = \pm 1$. However, $\hat{Z}_k$ may stand for, e.g., the position operator, in which case the field $s_{i,\tau}$ is a continuous real field.
\end{itemize}
