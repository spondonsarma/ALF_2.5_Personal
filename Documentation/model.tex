...
\item In general $\hat{Z}_k$ can denote different operators and we can work, for instance, in a basis for the Ising spins where $\hat{Z}_k$ is diagonal: $\hat{Z}_{k}|s_{k}\rangle = s_{k}|s_{k}\rangle$, where $s_{k}=\pm 1$.
...
\end{equation}
Here, the HS fields take the values $l_{j,\tau} = \pm 2, \pm 1 $ and, for $\hat{Z}_k$  corresponding to the Pauli matrix $\hat{\sigma}_{z}$, the Ising spins take the values $s_{i,\tau} = \pm 1$. The field $s_{i,\tau}$ can however also be a continuous real field when the operator $\hat{Z}_k$ corresponds, e.g., to the position operator.
\end{itemize}
...
&\quad   \prod\limits_{s=1}^{N_{\mathrm{fl}}} 
   \left[
    ...
    \prod_{k=1}^{M_V}   e^{  \sqrt{ -\Delta \tau  U_k} \eta_{k,\tau} {\bm V}^{(ks)} }   \prod_{k=1}^{M_I}   e^{  -\Delta \tau s_{k,\tau}  {\bm I}^{(ks)}}  
      \prod_{k=1}^{M_T}   e^{-\Delta \tau \textbf{T}^{(ks)}}   \right) \right]^{N_{\mathrm{col}}}\;,
\end{align}
