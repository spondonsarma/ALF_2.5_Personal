...
The code that we describe below  can be found in the module \path{Prog/Hamiltonians/Hamiltonian_plain_vanilla_hubbard_mod.F90}.   Editing this file may be a good starting point to implement a new model Hamiltonian. 
...
%------------------------------------------------------------
\subsection{The hopping: \texttt{Call Ham\_hop}}
%------------------------------------------------------------
...
\end{lstlisting} 
Be aware that \texttt{Obser_Latt_make} does not copy the Bravais lattice and unit cell  \texttt{Latt} and \texttt{Latt_unit}, but links them through pointers to be more memory efficient. One can have different lattices attached to different observables by declaring additional instances of \texttt{Type(Lattice)} and \texttt{Type(Unit_cell)}.
 For equal-time correlation functions, we set \texttt{Nt = 1}. In this chase, \textt{Channel} does not have a meaning.
 If \texttt{Ltau = 1}, then the code will allocate space for time displaced quantities. The same structure as for equal-time correlation functions will be used, albeit with  \texttt{Nt = Ltrot + 1} and the channel should be set. Whith \textt{Channel="PH"}, the analysis will assume the observable to be particel-hole symmetric. For more details on the meaning of this parameter, see Sec.~\ref{sec:maxent}.
...
%-------------------------------------------------------------------------------------
\subsubsection{Measuring time displaced observables: \texttt{ObserT(NT, GT0, G0T, G00, GTT, PHASE)} }  \label{sec:TimeDispObs}
%-------------------------------------------------------------------------------------
%
...
\end{lstlisting}
\red{[Should we declare Run.py legacy, and substitute it by dedicated python scripts?]}
