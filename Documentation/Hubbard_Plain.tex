...
\begin{enumerate}
\item create the file \path{Prog/Hamiltonians/Hamiltonian_##NAME##_smod.F90} and
\item add the line \path{##NAME##} to \path{Prog/Hamiltonians.list}.
\end{enumerate}
Where \path{##NAME##} has to be replaced by the designation of the new model, in this example \path{Hubbard_Plain_Vanilla}. This can then later be selected through the parameter \path{ham_name}. There is a template \path{Prog/Hamiltonians/Hamiltonian_##NAME##_smod.F90} meant to be copied for creating a new model.

To get a valid Hamiltonian, one has to specify the parameters, the lattice, the hopping, the interaction, the trial wave function  (if  required), and the observables.  Consider  the  \textit{plain vanilla}  Hubbard model  written as: 
\begin{equation}
