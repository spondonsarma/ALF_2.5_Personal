...
%------------------------------------------------------------
%  
The code allows for global updates as well. The user must then provide two additional functions in the module \texttt{Hamiltonian\_Examples.F90}: \texttt{Global\_move} and \texttt{Delta\_\-S0\_\-global(Nsigma\_old)}.   


The subroutine  \texttt{Global\_move(T0\_Proposal\_ratio,nsigma\_old,size\_clust)}  proposes  a global move. 
Its single input is the two-dimensional array \texttt{nsigma\_old(M\_V + M\_I, Ltrot)} that contains  the full  configuration $C$.  On output, the new configuration $C'$, determined by the user,  is stored in the two-dimensional array \texttt{nsigma}, which is a global variable declared in the module \texttt{Hamiltonian}.
...
\texttt{Delta\_\-S0\_\-global(Nsigma\_old)} that computes the ratio $e^{-S_0(C')}/e^{-S_0(C)}$. Again, the configuration $C'$ is given by the array \texttt{nsigma(M\_V + M\_I, Ltrot)}, which is a global variable declared in the module \texttt{Hamiltonian}.
