...
Auxiliary field methods  can be formulated in very different ways.  The fields define  the  configuration space $\mathcal{C}$. They can stem from a Hubbard-Stratonovich (HS)  \cite{Hubbard59} transformation required to decouple the  many-body interacting term into a sum of non-interacting problems or they can correspond to  bosonic modes  with predefined dynamics such as a phonons.  In all cases, the result is that  the grand-canonical partition function  takes the form, 
\begin{equation}
	 Z = \text{Tr}\left( e^{-\beta \hat{\mathcal{H}}}\right)   =   \sum_{\mathcal{C}} e^{-S(\mathcal{C}) },
\end{equation}
where  $S$  is the action of non-interacting fermions subject to a  space-time fluctuating auxiliary field.    The high-dimensional  integration  over the fields is carried out stochastically.  They are many wasy of doing this.  For continuous fields one can opt for Langevin  dynamics or hybrid methods \cite{Duane85}.  Many problems can however can however be formulated with discrete fields. In this case,   Monte Carlo importance sampling will often be put to use \cite{Sokal89}.   Formulations do not differ  only  by the choice of the fields, continuous or discrete,  but also by the  formulation of the action itself.
