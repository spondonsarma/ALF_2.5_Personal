...
!--------------------------------------------------------------------
    ...
    submodule (Hamiltonian_main) ham_Kondo_Heisenberg_smod
      ...
      type, extends(ham_base) :: ham_Kondo_Heisenberg
      contains
      ...
      !#PARAMETERS START# VAR_Kondo_Heisenberg
      ...
      real(Kind=Kind(0.d0)) :: Ham_JK   = 2.d0  ! Kondo_Heisenberg Coupling  J
      real(Kind=Kind(0.d0)) :: Ham_Jh   = 2.d0  ! Heisenberg  coupling for the f-electrons
      Integer               :: Ham_dphi = 1     ! Delta_phi 
      !#PARAMETERS END#
      ...
      Integer, allocatable   :: Field_List_Kondo_Heisenberg(:,:,:)
      ...
      Type (Unit_cell), Target  :: Latt_unit_d    ! "Unit cell"  for  dimer correlations 
      Integer, allocatable      :: List_c(:,:), Invlist_c(:,:)  
      Integer, allocatable      :: List_f(:,:), Invlist_f(:,:)  
      Integer, allocatable      :: List_bonds  (:,:) !  Bonds  for Heisenberg interaction
      Integer, allocatable      :: List_bonds_c(:,:) !  Bonds  for calculating kin-kin-correlations

    contains
      
      module Subroutine Ham_Alloc_Kondo_Heisenberg
        allocate(ham_Kondo_Heisenberg::ham)
      end Subroutine Ham_Alloc_Kondo_Heisenberg
...
#include "Hamiltonian_Kondo_Heisenberg_read_write_parameters.F90"
...
!--------------------------------------------------------------------
      Subroutine Ham_Set
          ...
          ! From dynamically generated file "Hamiltonian_Kondo_Heisenberg_read_write_parameters.F90"
          call read_parameters()
          
          If ( .not. ( Lattice_type == "Bilayer_square" .or.  Lattice_type == "Bilayer_honeycomb") ) then
             Write(error_unit,*) "The Kondo_Heisenberg Hamiltonian is only defined for bilayer lattices"
          ...
          IF ( N_FL > 1 ) then
             Write(error_unit,*) 'For the Kondo_Heisenberg systems, N_FL has  to be equal to unity'
             CALL Terminate_on_error(ERROR_HAMILTONIAN,__FILE__,__LINE__)
          Endif
          ...
          If (Irank_g == 0) then
             ...
             Write(unit_info,*) '====================================='
             Write(unit_info,*) 'Model is      : Kondo_Heisenberg'
             ...
             Write(unit_info,*) 'Ham_Jh        : ', Ham_Jh
             Write(unit_info,*) 'Ham_chem      : ', Ham_chem
             Write(unit_info,*) 'Ham_dphi      : ', Ham_dphi
             if (Projector) then
                Do nf = 1,N_FL
...
!--------------------------------------------------------------------
        Subroutine Ham_Latt
          ...
          Implicit none
          Integer :: n, nc,I, no, nb, I1,I2, I1_c,  I2_c
          ...
          Case ("Bilayer_square")
             ...
             Latt_Unit_d%Norb    =  Latt_Unit_f%N_coord    
             Allocate (Latt_Unit_d%Orb_pos_p(Latt_Unit_d%Norb,3))
             Latt_Unit_d%Orb_pos_p = 0.d0
          ...
          Allocate ( List_bonds  ( Latt%N*Latt_Unit_f%N_coord, 3 ) )
          Allocate ( List_bonds_c( Latt%N*Latt_Unit_f%N_coord, 3 ) )
          Select case (Lattice_type)
          Case ("Bilayer_square")
             nb = 0
             do no =  1, Latt_Unit_f%N_coord
                do I = 1,Latt%N
                   nb  = nb + 1
                   I1    =   Invlist_f(I,1)
                   I1_c  =   Invlist_c(I,1)
                   if  (no == 1 )  then
                      I2    =  Invlist_f(latt%nnlist(I,1,0),1)
                      I2_c  =  Invlist_c(latt%nnlist(I,1,0),1)
                      List_bonds( nb, 3 ) = 1
                   elseif ( no == 2 )  then
                      I2   =  Invlist_f(latt%nnlist(I,0,1),1)
                      I2_c =  Invlist_c(latt%nnlist(I,0,1),1)
                      List_bonds( nb, 3 ) = 2
                   endif
                   List_bonds  ( nb, 1 ) = I1
                   List_bonds  ( nb, 2 ) = I2

                   List_bonds_c( nb, 1 ) = I1_c
                   List_bonds_c( nb, 2 ) = I2_c
             ...
             do no =  1, Latt_Unit_f%N_coord
                do I = 1,Latt%N
                   ...
                   if     (no == 1 )  then
                      ...
                      List_bonds( nb, 3 ) = 1
                   elseif (no == 2 )  then
                      I2   =  Invlist_f(latt%nnlist(I,1,-1),2)
                      I2_c =  Invlist_c(latt%nnlist(I,1,-1),2)
                      List_bonds( nb, 3 ) = 2
                   else
                      I2   =  Invlist_f(latt%nnlist(I,0,-1),2)
                      I2_c =  Invlist_c(latt%nnlist(I,0,-1),2)
                      List_bonds( nb, 3 ) = 3
                   endif
                   List_bonds( nb, 1 ) = I1
                   List_bonds( nb, 2 ) = I2
...
!--------------------------------------------------------------------
        Subroutine Ham_Hop
          ...
          Real (Kind=Kind(0.d0) ), allocatable :: Ham_T_generic(:,:), Ham_Tperp_generic(:,:)
...
!--------------------------------------------------------------------
        Subroutine Ham_V
          ...
          Integer :: nf, I, I1, I2,  nc,  no, N_ops, nb
          ...
          If  (   .not. Symm  ) then 
             ...
             if (abs(Ham_JK) > Zero )  Then
                if (N_SUN == 2 ) then
                   ...
                   allocate( Field_List_Kondo_Heisenberg(Latt%N,Latt_Unit%Norb,1) )
                elseif (N_SUN > 2  )  then
                   N_ops = N_ops + Latt%N*Latt_Unit%Norb*2/2
                   allocate( Field_List_Kondo_Heisenberg(Latt%N,Latt_Unit%Norb,2) )
                endif
             endif
             if (abs(Ham_Jh) > Zero  )  Then
                if (N_SUN == 2 ) then
                   N_ops = N_ops + Latt%N*Latt_Unit_f%N_coord 
                elseif (N_SUN > 2  )  then
                   N_ops = N_ops + Latt%N*Latt_Unit_f%N_coord*2 
                endif
             endif
                ...
                Do I = 1,Latt%N
                   Do no = 1, Latt_unit%Norb/2
                      ...
                      if (N_SUN == 2 ) then
                         ...
                         Field_List_Kondo_Heisenberg(I,no,1) = nc
                      else
                         nc = nc + 1
                         ...
                         Field_List_Kondo_Heisenberg(I,no,2) = nc
                      endif
                   Enddo
                Enddo
             endif
             if ( abs(Ham_Jh)  > Zero ) then
                do nb =   1, Size(List_bonds,1)
                   I1 =  List_bonds(nb,1)
                   I2 =  List_bonds(nb,2)
                   if (N_SUN == 2 ) then
                      nc = nc + 1
                      Call Predefined_Int_V_SUN ( OP_V(nc,1), I1,I2, N_SUN, DTAU     , Ham_Jh/2.d0 )
                   else
                      nc = nc + 1
                      Call Predefined_Int_V_SUN ( OP_V(nc,1), I1,I2, N_SUN, DTAU     , Ham_Jh/4.d0 )
                      nc = nc + 1
                      Call Predefined_Int_VJ_SUN( OP_V(nc,1), I1,I2, N_SUN, DTAU     , Ham_Jh/4.d0 )
                   endif
                enddo
             endif
          elseif (Symm)  then 
             if (abs(Ham_Uc) > Zero  )  N_ops = N_ops + Latt%N*Latt_Unit%Norb*2/2
             if (abs(Ham_Uf) > Zero  )  N_ops = N_ops + Latt%N*Latt_Unit%Norb/2
             if (abs(Ham_JK) > Zero  )  Then
                if (N_SUN == 2 ) then
                   N_ops = N_ops + Latt%N*Latt_Unit%Norb*2/2
                   allocate( Field_List_Kondo_Heisenberg(Latt%N,Latt_Unit%Norb,1) )
                elseif (N_SUN > 2  )  then
                   N_ops = N_ops + Latt%N*Latt_Unit%Norb*4/2
                   allocate( Field_List_Kondo_Heisenberg(Latt%N,Latt_Unit%Norb,2) )
                endif
             endif
             if (abs(Ham_Jh) > Zero  )  Then
                if (N_SUN == 2 ) then
                   N_ops = N_ops + Latt%N*Latt_Unit_f%N_coord*2 
                elseif (N_SUN > 2  )  then
                   N_ops = N_ops + Latt%N*Latt_Unit_f%N_coord*4 
                endif
             endif
             Allocate(Op_V(N_ops,N_FL))
             nc = 0
             if ( abs(Ham_JK)  > Zero ) then
                Do I = 1,Latt%N
                   Do no = 1, Latt_unit%Norb/2
                      ...
                      if (N_SUN == 2 ) then
                         nc = nc + 1
                         Call Predefined_Int_V_SUN ( OP_V(nc,1), I1,I2, N_SUN, DTAU/2.d0   , Ham_JK/2.d0 )
                      else
                         nc = nc + 1
                         Call Predefined_Int_V_SUN ( OP_V(nc,1), I1,I2, N_SUN, DTAU/2.d0   , Ham_JK/4.d0 )
                         Field_List_Kondo_Heisenberg(I,no,1) = nc
                         nc = nc + 1
                         Call Predefined_Int_VJ_SUN( OP_V(nc,1), I1,I2, N_SUN, DTAU/2.d0   , Ham_JK/4.d0 )
                         Field_List_Kondo_Heisenberg(I,no,2) = nc
                      endif
                   Enddo
                Enddo
             ...
             if ( abs(Ham_Jh)  > Zero ) then
                do nb =   1, Size(List_bonds,1)
                   ...
                   if (N_SUN == 2 ) then
                      nc = nc + 1
                      Call Predefined_Int_V_SUN ( OP_V(nc,1), I1,I2, N_SUN, DTAU/2.d0     , Ham_Jh/2.d0 )
                   else
                      nc = nc + 1
                      Call Predefined_Int_V_SUN ( OP_V(nc,1), I1,I2, N_SUN, DTAU/2.d0     , Ham_Jh/4.d0 )
                      nc = nc + 1
                      Call Predefined_Int_VJ_SUN( OP_V(nc,1), I1,I2, N_SUN, DTAU/2.d0    , Ham_Jh/4.d0 )
                   endif
                enddo
             endif
             if ( abs(Ham_Uc)  > Zero ) then
                Do I = 1,Latt%N
                   do no = 1, Latt_unit%Norb/2
                      I1 = invlist(I,no)
                      nc = nc + 1
                      Call Predefined_Int_U_SUN(  OP_V(nc,1), I1, N_SUN, DTAU/2.d0, Ham_Uc )
                   enddo
                Enddo
             Endif
             if ( abs(Ham_Uf)  > Zero ) then
        ...
        Subroutine  Alloc_obs(Ltau)
          ...
          Do I = 1,Size(Obs_eq,1)
             ...
             elseif ( I ==  7 .or. I == 8 ) then
                Call Obser_Latt_make(Obs_eq(I), Nt, Filename, Latt, Latt_unit_d, Channel, dtau)
             else
          ...
          If (Ltau == 1) then
             ...
             Do I = 1,Size(Obs_tau,1)
                ...
                case (6)
                   Channel = 'P' ; Filename = "GreenfT"
                case (7)
                   Channel = 'PH' ; Filename = "HybT"
                case default
                   Write(6,*) ' Error in Alloc_obs '
                end select
                Nt = Ltrot+1-2*Thtrot
...
!-------------------------------------------------------------------
        subroutine Obser(GR,Phase,Ntau, Mc_step_weight)
          ...
          !Local
          Complex (Kind=Kind(0.d0)) :: GRC(Ndim,Ndim,N_FL), ZK, Z_I,  Z_J, Z_II
          Complex (Kind=Kind(0.d0)) :: Zrho, Zkin, Zhubc, ZCon, ZJ, Z, ZP,ZS, ZZ, ZXY,  Zkin_ref
          Integer :: I,J, no, n, I_c,I_f, nf, J_c, J_f, no_I, no_J, imj, dphi, ntau1
          Integer :: I1, I2,  J1,J2, nb, nb1
          Integer :: I1_c, I2_c,  J1_c,  J2_c,  no_I_c,  no_J_c
          Real    (Kind=Kind(0.d0)) :: X
          Integer,      allocatable ::  Nphi_ref(:)
          Complex (Kind=Kind(0.d0)),  allocatable ::    Z_tmp(:,:)
          ...
          Allocate  (Nphi_ref(N_FL) ) 
          Zkin_ref       =   Zkin
          do  nf = 1,N_FL
             Nphi_ref(nf)   =   Hopping_Matrix(nf)%N_phi
          enddo
          Do dphi = 1, Size( Obs_scal(6)%Obs_vec )
             do  nf  =  1,N_Fl
                Hopping_Matrix(nf)%N_phi  =  Nphi_ref(nf)  +  Ham_dphi 
             enddo
             Zkin = cmplx(0.d0, 0.d0, kind(0.D0))
             Call Predefined_Hoppings_Compute_Kin(Hopping_Matrix,List,Invlist, Latt, Latt_unit, GRC, ZKin)
             Zkin = Zkin* dble(N_SUN)
             Obs_scal(6)%Obs_vec(dphi)  =    Obs_scal(6)%Obs_vec(dphi) + (Zkin - Zkin_ref)*ZP* ZS/real(Ham_dphi,kind(0.d0))
          enddo
          do  nf  =  1,N_Fl
             Hopping_Matrix(nf)%N_phi  =   Nphi_ref(nf) 
          enddo
          deallocate  (Nphi_ref ) 
          ...
          ! Dimer_f correlations
          obs_eq(7)%N        = obs_eq(7)%N + 1
          obs_eq(7)%Ave_sign = obs_eq(7)%Ave_sign + real(ZS,kind(0.d0))
          ! Dimer_Kin_c correlations
          obs_eq(8)%N        = obs_eq(8)%N + 1
          obs_eq(8)%Ave_sign = obs_eq(8)%Ave_sign + real(ZS,kind(0.d0))
          ! Use  the  bond list  that you have  defined for the Heisenberg  interaction.
          do nb  =  1, Latt%N*Latt_Unit_f%N_coord
             I1   = List_bonds( nb, 1 )
             I2   = List_bonds( nb, 2 )
             no_I = List_bonds( nb, 3 )
             I1_c   = List_bonds_c( nb, 1 )
             I2_c   = List_bonds_c( nb, 2 )
             no_I_c = List_bonds_c( nb, 3 )
             I    = List(I1  ,1) !  Unit cell
             I_c  = List(I1_c,1) !  Unit cell
             !if ( I_c  /=  I )  then
             !   write(6,*) 'Error'
             !   stop
             !endif
             do nb1  = 1,  Latt%N*Latt_Unit_f%N_coord
                J1   = List_bonds( nb1, 1 )
                J2   = List_bonds( nb1, 2 )
                no_J = List_bonds( nb1, 3 )
                J1_c   = List_bonds_c( nb1, 1 )
                J2_c   = List_bonds_c( nb1, 2 )
                no_J_c = List_bonds_c( nb1, 3 )
                J     = List(J1  ,1) !  Unit  cell
                J_c   = List(J1_c,1) !  Unit  cell
                Imj = latt%imj(I,J)
                Z   = Predefined_Obs_dimer_eq(I1,I2,J1,J2, GR, GRC, N_SUN, N_FL) 
                obs_eq(7)%Obs_Latt(imj,1,no_I,no_J) =  Obs_eq(7)%Obs_Latt(imj,1,no_I,no_J) + Z*ZP*ZS

                Z   = Predefined_Obs_dimer_kin_eq(I1_c,I2_c,J1_c,J2_c, GR, GRC, N_SUN, N_FL) 
                obs_eq(8)%Obs_Latt(imj,1,no_I,no_J) =  Obs_eq(8)%Obs_Latt(imj,1,no_I,no_J) + Z*ZP*ZS

             enddo
             Obs_eq(7)%Obs_Latt0(no_I) =  Obs_eq(7)%Obs_Latt0(no_I) +  &
                     &  Predefined_Obs_dimer0_eq(I1,I2, GR, N_SUN, N_FL) * ZP*ZS

             Obs_eq(8)%Obs_Latt0(no_I) =  Obs_eq(8)%Obs_Latt0(no_I) +  &
                  &  Predefined_Obs_dimer_kin0_eq(I1_c,I2_c, GR, N_SUN, N_FL) * ZP*ZS

          enddo


          
          ! Tilde-f,   Gauge  invariant  hybredization
          ...
          If  ( abs(Ham_Jk) > 0.d0 )   then
             Allocate  (Z_tmp(Latt%N,  Latt_unit%Norb / 2 ))
             Do I = 1,Latt%N
                do no_I =  1, Latt_unit%Norb/2
                   If (N_SUN == 2)  then
                      Z_I  = cmplx(nsigma%Phi(Field_list_Kondo_Heisenberg(I,no_I,1), ntau ), 0.d0, Kind(0.d0)) 
                   else
                      Z_I  = cmplx(nsigma%Phi(Field_list_Kondo_Heisenberg(I,no_I,1), ntau ), 0.d0, Kind(0.d0)) + &
                           & cmplx(0.d0,nsigma%Phi(Field_list_Kondo_Heisenberg(I,no_I,2), ntau ), Kind(0.d0))
                   endif
                   Z_tmp(I,no_I)  =  Z_I/sqrt(Z_I*conjg(Z_I))
                   Z_tmp(I,no_I)  =  Z_tmp(I,no_I)/( 2.d0*Op_V(Field_list_Kondo_Heisenberg(I,no_I,1),1)%g )
                enddo
             enddo
             Do I = 1,Latt%N
                do no_I  = 1, Latt_unit%Norb / 2
                   ...
                   Do J = 1,Latt%N
                      ...
                      do no_J  = 1, Latt_unit%Norb / 2
                         ...
                         if ( I_f == J_f ) then
                            ...
                            Z  = GRC(I_f,J_f,1) * conjg(Z_tmp(I,no_I)) * Z_tmp(J,no_J)
                         endif
                         obs_eq(5)%Obs_Latt(imj,1,no_I,no_J) =  Obs_eq(5)%Obs_Latt(imj,1,no_I,no_J) + Z*ZP*ZS
                         
                         if ( I_f == J_f ) then
                            Z  =   (GRC(I_c,I_f,1) +  GRC(I_f,I_c,1))**2  + &
                                 &  GRC(I_c,J_f,1)*GR(I_f,J_c,1)          + &
                                 &  GRC(I_c,J_c,1)*GR(I_f,J_f,1)          + &
                                 &  GRC(I_f,J_f,1)*GR(I_c,J_c,1)          + &
                                 &  GRC(I_f,J_c,1)*GR(I_c,J_f,1)
                         else
                            Z  =   (Z_tmp(I,no_I)*GRC(I_c,I_f,1) +  conjg(Z_tmp(I,no_I))*GRC(I_f,I_c,1)) * &
                                 & (Z_tmp(J,no_J)*GRC(J_c,J_f,1) +  conjg(Z_tmp(J,no_J))*GRC(J_f,J_c,1)) + &
                                 &  Z_tmp(I,no_I)*Z_tmp(J,no_J)       *GRC(I_c,J_f,1)*GR(I_f,J_c,1)      + &
                                 &  Z_tmp(I,no_I)*conjg(Z_tmp(J,no_J))*GRC(I_c,J_c,1)*GR(I_f,J_f,1)      + &
                                 &  conjg(Z_tmp(I,no_I))*Z_tmp(J,no_J)*GRC(I_f,J_f,1)*GR(I_c,J_c,1)      + &
                                 &  conjg(Z_tmp(I,no_I)*Z_tmp(J,no_J))*GRC(I_f,J_c,1)*GR(I_c,J_f,1)
                         endif
                         obs_eq(6)%Obs_Latt(imj,1,no_I,no_J) =  Obs_eq(6)%Obs_Latt(imj,1,no_I,no_J) + Z*ZP*ZS
                      enddo
                   enddo
                   Z  =   Z_tmp(I,no_I)*GRC(I_c,I_f,1) +  conjg(Z_tmp(I,no_I))*GRC(I_f,I_c,1)
                   Obs_eq(6)%Obs_Latt0(no_I) =  Obs_eq(6)%Obs_Latt0(no_I) +  Z * ZP*ZS
                enddo
             enddo
...
!-------------------------------------------------------------------
        Subroutine ObserT(NT,  GT0,G0T,G00,GTT, PHASE,Mc_step_weight)
          ...
          !Locals
          Complex (Kind=Kind(0.d0)) :: Z, ZP, ZS, ZZ, ZXY, Z_I,  Z_J, Z_II
          Real    (Kind=Kind(0.d0)) :: X
          Integer :: IMJ, I_c, I_f, J_c, J_f, I,J, no_I, no_J, NTP1
          ...
          If (NT == 0 ) then
             ...
             obs_tau(6)%N        = obs_tau(6)%N + 1
             obs_tau(6)%Ave_sign = obs_tau(6)%Ave_sign + real(ZS,kind(0.d0))
             obs_tau(7)%N        = obs_tau(7)%N + 1
             obs_tau(7)%Ave_sign = obs_tau(7)%Ave_sign + real(ZS,kind(0.d0))
          endif
          NTP1 =  NT + 1
          If (NT ==  Ltrot )  NTP1 = 1
          Do I = 1,Latt%N
             do no_I  = 1, Latt_unit%Norb / 2
                ...
                I_f = Invlist(I,no_I + Latt_unit%Norb / 2 )
                If (N_SUN == 2)  then
                   Z_I  = cmplx(nsigma%Phi(Field_list_Kondo_Heisenberg(I,no_I,1), NTP1 ), 0.d0, Kind(0.d0)) 
                else
                   Z_I  = cmplx(nsigma%Phi(Field_list_Kondo_Heisenberg(I,no_I,1), NTP1 ), 0.d0, Kind(0.d0)) + &
                        & cmplx(0.d0,nsigma%Phi(Field_list_Kondo_Heisenberg(I,no_I,2), NTP1 ), Kind(0.d0))
                endif
                Z_I  = Z_I/sqrt(Z_I*conjg(Z_I))
                Z_I  = Z_I/( 2.d0*Op_V(Field_list_Kondo_Heisenberg(I,no_I,1),1)%g )
                Do J = 1,Latt%N
                   Imj = latt%imj(I,J)
                   do no_J  = 1, Latt_unit%Norb / 2
                      J_c = Invlist(J,no_J)
                      J_f = Invlist(J,no_J + Latt_unit%Norb / 2 )
                      If (N_SUN == 2)  then
                         Z_J  = cmplx(nsigma%Phi(Field_list_Kondo_Heisenberg(J,no_J,1), 1 ), 0.d0, Kind(0.d0)) 
                      else
                         Z_J  = cmplx(nsigma%Phi(Field_list_Kondo_Heisenberg(J,no_J,1), 1 ), 0.d0, Kind(0.d0)) + &
                              & cmplx(0.d0,nsigma%Phi(Field_list_Kondo_Heisenberg(J,no_J,2), 1 ), Kind(0.d0))
                      endif
                      Z_J  = Z_J/sqrt(Z_J*conjg(Z_J))
                      Z_J  = Z_J/( 2.d0*Op_V(Field_list_Kondo_Heisenberg(J,no_J,1),1)%g )
                      ...
                      If ( I_f == J_f  .and.  ( NT == 0 .or. NT ==  Ltrot ) )  then
                         Z  = GT0(I_f,J_f,1) 
                      else
                         Z  = GT0(I_f,J_f,1)*Z_I*conjg(Z_J)
                      endif
                      obs_tau(6)%Obs_Latt(imj,NT+1,no_I,no_J) =  Obs_tau(6)%Obs_Latt(imj,NT+1,no_I,no_J) + Z*ZP*ZS


                      if ( I_f == J_f .and. ( NT == 0 .or. NT ==  Ltrot ) ) then
                         Z  =    (-GTT(I_c,I_f,1) -  GTT(I_f,I_c,1))**2       &
                              &  - GTT(J_f,I_c,1)          * GTT(I_f,J_c,1)   &
                              &  + (1.d0 - GTT(J_c,I_c,1)) * GTT(I_f,J_f,1)   &
                              &  + (1.d0 - GTT(J_f,I_f,1)) * GTT(I_c,J_c,1)   &
                              &  - GTT(J_c,I_f,1)*GTT(I_c,J_f,1)
                      else
                         Z  =   (-Z_I*GTT(I_f,I_c,1) -  conjg(Z_I)*GTT(I_c,I_f,1)) * &
                              & (-Z_J*G00(J_f,J_c,1) -  conjg(Z_J)*G00(J_c,J_f,1)) - &
                              &  Z_I*Z_J       *G0T(J_f,I_c,1)*GT0(I_f,J_c,1)      - &
                              &  Z_I*conjg(Z_J)*G0T(J_c,I_c,1)*GT0(I_f,J_f,1)      - &
                              &  conjg(Z_I)*Z_J*G0T(J_f,I_f,1)*GT0(I_c,J_c,1)      - &
                              &  conjg(Z_I*Z_J)*G0T(J_c,I_f,1)*GT0(I_c,J_f,1)
                      endif
                ...
                Z = (-Z_I*GTT(I_f,I_c,1) -  conjg(Z_I)*GTT(I_c,I_f,1))
                Obs_tau(7)%Obs_Latt0(no_I) =  Obs_tau(7)%Obs_Latt0(no_I) +  Z*ZP*ZS
             enddo
          enddo
...
!--------------------------------------------------------------------
    ...
    end submodule ham_Kondo_Heisenberg_smod
