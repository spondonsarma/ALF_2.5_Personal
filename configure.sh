...
PRECOMPILER_LINEINFO="-I${Libs}/headerfiles"
# PRECOMPILER_LINEINFO="-D'Terminate_on_error(err)=Terminate_on_error_with_lineinfo(err,__FILE__,__LINE__)'"
# Modules need to know the programm configuration since entanglement needs MPI
ALF_FLAGS_MODULES="${F90OPTFLAGS} ${PROGRAMMCONFIGURATION} ${ALF_FLAGS_EXT} ${PRECOMPILER_LINEINFO}"
ALF_FLAGS_ANA="${F90USEFULFLAGS} ${F90OPTFLAGS} ${ALF_INC} ${ALF_FLAGS_EXT} ${PRECOMPILER_LINEINFO}"
ALF_FLAGS_PROG="${F90USEFULFLAGS} ${F90OPTFLAGS} ${PROGRAMMCONFIGURATION} ${ALF_INC} ${ALF_FLAGS_EXT} ${PRECOMPILER_LINEINFO}"
